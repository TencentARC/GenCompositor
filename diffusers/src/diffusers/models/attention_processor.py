...
class Attention_cross(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,
        cross_states: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        r"""
        The forward method of the `Attention` class.
...
class CogVideoXAttnProcessor2_0:
    ...
    def __call__(
        ...
        # encoder_hidden_states: [2, 226, 3072] | hidden_states: [2, 17550, 3072],  49//4 * 480//16 * 720//16

        hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)    # [2, 17776, 3072]
        ...
        query = attn.to_q(hidden_states)    # [2, 17776, 3072]
        key = attn.to_k(hidden_states)      # [2, 17776, 3072]
        value = attn.to_v(hidden_states)    # [2, 17776, 3072]

        inner_dim = key.shape[-1]
        head_dim = inner_dim // attn.heads

        query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)        # [2, 48, 17776, 64]
        key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)            # [2, 48, 17776, 64]
        value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)        # [2, 48, 17776, 64]
        ...
        # image_rotary_emb0: [17550, 64] | image_rotary_emb1: [17550, 64]
        if image_rotary_emb is not None:
            from .embeddings import apply_rotary_emb
        ...
        # [1, 48, 35326, 64]  或  [1, 48, 17776, 64]
        hidden_states = hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)

        # linear proj
        hidden_states = attn.to_out[0](hidden_states)
        # dropout
        hidden_states = attn.to_out[1](hidden_states)           # [1, 35326, 3072]  或  [1, 17776, 3072]
...
class CogVideoXAttnProcessor2_0_cross:
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """
    ...
    def __call__(
        ...
        # hidden_states: [1, 17550, 3072] | cross_states: [1, 17550, 3072]
        query = attn.to_q(hidden_states)
        key = attn.to_k(cross_states)
        value = attn.to_v(cross_states)
        ...
        if prev_hidden_states is not None and prev_clip_weight is not None and prev_clip_weight > 0.0:
            ...
            if image_rotary_emb is not None:
                ...
                if not attn.is_cross_attention:
                    prev_key = apply_rotary_emb(prev_key, image_rotary_emb)
        else:
            prev_key = None
            prev_value = None
