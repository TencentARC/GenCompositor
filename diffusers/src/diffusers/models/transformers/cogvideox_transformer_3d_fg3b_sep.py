...
from ..embeddings import CogVideoXPatchEmbed_sep, TimestepEmbedding, Timesteps
...
class CogVideoXBlock(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,        # [2, 17550 + 17550, 3072]
        ...
        elif isinstance(self.processor, CogVideoXAttnProcessor2_0):     # 走这
            attn_hidden_states, attn_encoder_hidden_states = self.attn1(
                hidden_states=norm_hidden_states,
                encoder_hidden_states=norm_encoder_hidden_states,
                image_rotary_emb=image_rotary_emb,
...
class CogVideoXTransformer3D3BModel_sep(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def __init__(
        ...
        # 1. Patch embedding
        self.patch_embed = CogVideoXPatchEmbed_sep(
    ...
    def from_transformer(
        ...
        if load_weights_from_transformer:
            ...
            branch.patch_embed.fg_proj.weight.data=torch.nn.Parameter(conv_in_condition_weight[:,:config.in_channels//2,...])
            branch.patch_embed.fg_proj.bias.data=transformer.patch_embed.proj.bias

            branch.patch_embed.proj.weight.data=transformer.patch_embed.proj.weight
    ...
    def forward(
        ...
        batch_size, num_frames, channels, height, width = hidden_states.shape       # [1, 13, 48, 60, 90]
        ...
        # 2. Patch embedding
        hidden_states, masks = self.patch_embed(encoder_hidden_states, hidden_states, masks=branch_block_masks) # 走这里, 只是做好准备   [2, 17776 + 17550, 3072], [2, 17550, 1]
        masks = masks.repeat(1, 1, int(hidden_states.shape[-1] / masks.shape[-1]))          # [2, 17550, 3072]
        hidden_states = self.embedding_dropout(hidden_states)

        text_seq_length = encoder_hidden_states.shape[1]
        encoder_hidden_states = hidden_states[:, :text_seq_length]      # [2, 226, 3072]
        hidden_states = hidden_states[:, text_seq_length:]              # [2, 17550 + 17550, 3072]
        sep_size = hidden_states.size(1)//2
        ...
        for i, block in enumerate(self.transformer_blocks):
            ...
            fg_states = hidden_states[:, - sep_size:]
            hidden_states = hidden_states[:, :sep_size]
        ...
        if not self.config.use_rotary_positional_embeddings:
            ...
            hidden_states = self.norm_final(hidden_states)
            hidden_states = hidden_states[:, text_seq_length : text_seq_length + sep_size]
