...
class CogVideoXI2VTriInpaintPipeline_sep(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def prepare_mask_latents(
        ...
        return mask, masked_video_latents, fg_video_latents
    ...
    def _prepare_rotary_positional_embeddings(
        ...
        base_size_width = base_width // (self.vae_scale_factor_spatial * self.transformer.config.patch_size)
        base_size_height = base_height // (self.vae_scale_factor_spatial * self.transformer.config.patch_size)
        ...
        freqs_cos, freqs_sin = get_3d_rotary_pos_embed(
            embed_dim=self.transformer.config.attention_head_dim,   # 64
            crops_coords=grid_crops_coords,
            grid_size=(grid_height, grid_width),    # (30, 45)
            temporal_size=num_frames,               # 13
        )
    ...
    def __call__(
        ...
        mask, masked_video_latents, fg_video_latents = self.prepare_mask_latents(
            mask_condition,
            masked_video,
            fg_condition,
            batch_size * num_videos_per_prompt,
        ...
        sep_image_rotary_emb = (
            self._prepare_rotary_positional_embeddings(height * 2, width, latents.size(1), device, base_height=480*2)
            if self.transformer.config.use_rotary_positional_embeddings
            else None
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latent_video_input, masked_video_latents, fg_video_latents], dim=2)  # [1, 13, 32 + 16, 60, 90]
