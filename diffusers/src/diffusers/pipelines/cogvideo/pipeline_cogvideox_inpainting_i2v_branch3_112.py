...
from ...models import AutoencoderKLCogVideoX, CogVideoXTransformer3DModel, CogVideoXTransformer3D3BModel
...
class CogVideoXI2VTriInpaint112Pipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        transformer: CogVideoXTransformer3D3BModel,
    ...
    def prepare_mask_latents(
        ...
        mask_latents = rearrange(mask.squeeze(0), "1 T (H P) (W Q) -> 1 (P Q) T H W", P=8, Q=8)                      # (1, 64, 49, 60, 90)
        mask_latents = torch.nn.functional.interpolate(
            mask_latents,
            size=(
                (mask.shape[-3] - 1) // self.vae_scale_factor_temporal + 1, height // self.vae_scale_factor_spatial, width // self.vae_scale_factor_spatial), mode='nearest-exact'
            ).to(device=device, dtype=dtype)       # [1, 64, 13, 60, 90]
        ...
        if masked_video.shape[1] == 4:
            ...
            fg_video_latents = self._encode_vae_video(fg_video, generator=generator)
        ...
        if mask.shape[0] < batch_size:
            ...
            mask_latents = mask_latents.repeat(batch_size // mask_latents.shape[0], 1, 1, 1, 1)
        ...
        fg_video_latents = (
            torch.cat([fg_video_latents] * 2) if do_classifier_free_guidance else fg_video_latents
        )

        # aligning device to prevent device errors when concating it with the latent model input
        masked_video_latents = masked_video_latents.to(device=device, dtype=dtype)
        fg_video_latents = fg_video_latents.to(device=device, dtype=dtype)
        return mask, masked_video_latents, fg_video_latents, mask_latents
    ...
    def __call__(
        ...
        fg_video: List[PIL.Image.Image] = None,
        ...
        # empty_encoder_hidden_states = self._get_t5_prompt_embeds(
        #     prompt="",
        #     num_videos_per_prompt=num_videos_per_prompt,
        #     max_sequence_length=max_sequence_length,
        #     device=device,
        ...
        if do_classifier_free_guidance:
            ...
            # empty_encoder_hidden_states = torch.cat([empty_encoder_hidden_states, empty_encoder_hidden_states], dim=0)
        ...
        init_video = self.video_processor.preprocess_video(original_video, height=height, width=width).to(device, dtype=torch.float32)  # [1, 3, 49, 480, 720]
        masked_video = self.video_processor.preprocess_video(masked_video, height=height, width=width).to(device, dtype=torch.float32)  # [1, 3, 49, 480, 720]
        fg_condition = self.video_processor.preprocess_video(fg_video, height=height, width=width).to(device, dtype=torch.float32)
        ...
        if return_video_latents:
            latents, noise, video_latents = latents_outputs      # [1, 13, 16, 60, 90]
        else:
            latents, noise = latents_outputs
        
        
        # 7. Prepare mask latent variables
        mask_condition = self.masked_video_processor.preprocess_video(mask_video, height=height, width=width)
        ...
        #     else:
        #         masked_video = init_video * (mask_condition < 0.5)        # 走这
        # else:
        #     masked_video = masked_video_latents

        mask, masked_video_latents, fg_video_latents, mask_latents = self.prepare_mask_latents(
            mask_condition,
            masked_video,
            fg_condition,
            batch_size * num_videos_per_prompt,
        ...
        mask_latents = mask_latents.permute(0, 2, 1, 3, 4)      # [2, 13, 64, 60, 90]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latent_video_input, fg_video_latents, masked_video_latents, mask_latents], dim=2)  # [1, 13, 32 + 16, 60, 90]
