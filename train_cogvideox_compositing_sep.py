...
if is_wandb_available():
    ...
    os.environ["WANDB_API_KEY"] = "320f1b3bcb18fb46ccbcfb196ec2e7b18e98e3f7"
...
class VideoInpaintingDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.load_mode == "online":
            ...
            n_frames = len(out) // pixels_per_frame
            ...
            masks = np.frombuffer(mask_out, np.uint8).reshape([-1, height, width, 1])[..., ::-1]
            masks = masks[start_frame:end_frame]
            mask_video = np.array(masks)
            mask_video = mask_video[::int(fps // self.fps)]

            masked_frames = np.frombuffer(masked_video_out, np.uint8).reshape([-1, height, width, 3])[..., ::-1]
            ...
            fg_frames = np.frombuffer(fg_out, np.uint8).reshape([-1, 576, 576, 3])[..., ::-1]
    ...
    def __call__(self, examples):
        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                pad_width = target_width - fg_resized.shape[-1]
                ...
                for i in range(frame):
                    ...
                    _, binary_mask = cv2.threshold(masks_video[i], 128, 255, cv2.THRESH_BINARY)
            ...
            for example in examples:
                ...
                pad_width = target_width - fg_resized.shape[2]
                pad_left = pad_width // 2
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                if global_step < 1:
                    ...
                    fg_values = cv2.cvtColor(fg_values, cv2.COLOR_BGR2RGB)
                ...
                images = batch["conditioning_pixel_values"][:, :1, :, :, :].permute(0, 2, 1, 3, 4)
                ...
                image_latents = vae.encode(
                    noisy_images.to(dtype=weight_dtype)).latent_dist.sample()

                image_latents = image_latents.permute(0, 2, 1, 3, 4) * vae.config.scaling_factor
                ...
                model_input = model_input.permute(0, 2, 1, 3, 4) * vae.config.scaling_factor
                ...
                inpainting_loss = torch.mean(
                    (weights * (model_pred * masks - target * masks) ** 2).reshape(batch_size, -1), dim=1)
